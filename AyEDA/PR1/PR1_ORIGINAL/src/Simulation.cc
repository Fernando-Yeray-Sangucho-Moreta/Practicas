...
void Simulation::hormiga_de_Lanton(std::ostream& out1, std::ostream& out2) {
  ...
  while(true) {
    ...
    if(coord_x < 0 || coord_x >= celdas_.size_x() || coord_y < 0 || coord_y >= celdas_.size_y()) {
      std::cout << "La hormiga se ha caido" << std::endl;
      break;
    }
    ...
    //if(contador%5 == 0) {std::cout << celdas_;}
    std::cout << celdas_;
    ...
    //if(contador%5 == 0) {
    //  int decision{0};
    //  std::cout << "Desea proseguir?(yes=1, no=0, guardar=2) ";
    //  std::cin >> decision;
    //  if(decision == 0) {
    //    break;
    //  } else if(decision == 2) {
    //    celdas_.write_no_format(out1);
    //  }
    //}
    int decision{0};
    std::cout << "Desea proseguir?(yes=1, no=0, guardar=2) ";
